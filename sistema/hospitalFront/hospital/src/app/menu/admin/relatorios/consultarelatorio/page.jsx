...
export default function ConsultaRelatorioPage() {
  ...
  const obterDataConsulta = (consulta) =>
    normalizarData(consulta?.date ?? consulta?.data ?? consulta?.createdAt ?? consulta?.created_at);
  ...
  const buscarConsultas = async () => {
    ...
    try {
      const resposta = await api.get("/consulta/get-all");
      const consultasCarregadas = Array.isArray(resposta.data?.data)
        ? resposta.data.data
      ...
      const filtradas = consultasCarregadas.filter((consulta) => {
        const dataConsulta = obterDataConsulta(consulta);
        return dataConsulta && dataConsulta >= inicio && dataConsulta <= fim;
      });
      ...
      <main className="relative flex flex-1 flex-col items-center overflow-hidden px-4 py-8">
        ...
        <div className="z-10 mt-10 flex w-full max-w-7xl flex-1 flex-col">
          <div className="mb-8 text-center">
            ...
            <p className="text-slate-600">Acompanhe as consultas registradas no sistema</p>
          </div>
                      ...
                      <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
                        <div className="space-y-1">
                          ...
                          <p className="text-base font-medium text-slate-800">
                            Paciente: {consulta?.paciente?.nome ?? consulta?.idPaciente ?? "Não informado"}
                          </p>
                          <p className="text-sm text-slate-500">
                            Data: {consulta?.date ?? consulta?.data ?? consulta?.createdAt ?? "Data não disponível"}
                          </p>
                        </div>

                        <div className="rounded-lg bg-slate-100 px-3 py-2 text-sm text-slate-600">
                          Médico: <span className="font-medium">{consulta?.medico?.nome ?? consulta?.idMedico ?? "N/A"}</span>
                        </div>
                      </div>

                      <div className="mt-4 grid gap-2 text-sm text-slate-600 md:grid-cols-2">
                        <p>
                          <strong>Triagem:</strong> {formatarValor(consulta?.triagem?.id ?? consulta?.idTriagem)}
                        </p>
                        <p>
                          <strong>Tipo:</strong> {consulta?.tipoConsulta ?? consulta?.tipo ?? "Consulta médica"}
                        </p>
                      </div>
